...
- [regtools](https://github.com/griffithlab/regtools) - Integrate DNA-seq and RNA-seq data to identify mutations that are associated with regulatory effects on gene expression. Identify evidence for aberrant splicing in RNA reads near a list of variants, Extract exon-exon junctions from a RNAseq BAM file, Annotate exon-exon junctions with information from a known transcriptome, Annotate variants with splice-region(the definition of this region is configurable) annotations. [Documentation](https://regtools.readthedocs.io/en/latest/)
